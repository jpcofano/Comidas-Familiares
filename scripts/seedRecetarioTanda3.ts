/**
 * Etapa E3.4.12 — Seed recetario tanda 3. admin SDK, SOLO-ALTA, idempotente.
 */
...
async function main() {
  ...
  writeFileSync(resolve(backupDir, `E3.4.12_ingredientes_${ts}.json`), JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2));
  writeFileSync(resolve(backupDir, `E3.4.12_recetas_${ts}.json`), JSON.stringify(recSnap.docs.map(d => d.data()), null, 2));
...
main().then(() => { console.log("Seed tanda 3 OK"); process.exit(0); }).catch((e) => { console.error(e); process.exit(1); });
