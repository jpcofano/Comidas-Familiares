/**
 * Etapa E3.4.10 — Seed recetario tanda 2. admin SDK (igual que tanda 1).
...
const NUEVOS_ING = JSON.parse(readFileSync(resolve("scripts/seed-data/ingredientes_nuevos_tanda2.json"), "utf-8")) as any[];
const NUEVAS_REC = JSON.parse(readFileSync(resolve("scripts/seed-data/recetas_nuevas_tanda2.json"), "utf-8")) as any[];
...
async function main() {
  ...
  writeFileSync(
    resolve(backupDir, `E3.4.10_ingredientes_${ts}.json`),
    JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2),
  );
  writeFileSync(
    resolve(backupDir, `E3.4.10_recetas_${ts}.json`),
    JSON.stringify(recSnap.docs.map(d => d.data()), null, 2),
  );
  console.log(`Backup guardado en scripts/backups/ (${ts})`);
  ...
  for (const def of NUEVOS_ING) {
    ...
    catByCanon.set(def.canonico, def.categoria);
    ingCre++;
...
main()
  .then(() => { console.log("Seed tanda 2 OK"); process.exit(0); })
