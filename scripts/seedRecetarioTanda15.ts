/**
 * Etapa E3.4.25 — Seed recetario tanda 15 tailandesa. admin SDK, SOLO-ALTA, idempotente.
 */
...
const NUEVOS_ING = JSON.parse(readFileSync(resolve(DIR, "ingredientes_nuevos_tanda15.json"), "utf-8")) as any[];
const NUEVAS_REC = JSON.parse(readFileSync(resolve(DIR, "recetas_tanda15.json"), "utf-8")) as any[];
...
async function main() {
  ...
  writeFileSync(resolve(backupDir, `E3.4.25_ingredientes_${ts}.json`), JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2));
  writeFileSync(resolve(backupDir, `E3.4.25_recetas_${ts}.json`), JSON.stringify(recSnap.docs.map(d => d.data()), null, 2));
  console.log(`Backup guardado (${ts})`);

  const idByCanon = new Map<string, string>();
  ingSnap.forEach((d) => { const x = d.data(); idByCanon.set(x.canonico, x.idIngrediente ?? d.id); });
  ...
  for (const def of NUEVOS_ING) {
    ...
    idByCanon.set(def.canonico, id); ingCre++;
  ...
  for (const r of NUEVAS_REC) {
    ...
    await db.collection("recetas").doc(id).set({ idReceta: id, nombreCanonico: nc, vecesCocinada: 0, fechaImportacion: FieldValue.serverTimestamp(), ...resto, ingredientes });
    recCre++; console.log(`  + ${id}  ${r.nombre}`);
  }
  console.log(`F3: ${recCre} creadas, ${recSalt} ya existían.`);
}

main().then(() => { console.log("Seed tanda 15 OK"); process.exit(0); }).catch((e) => { console.error(e); process.exit(1); });
