/**
 * Etapa E3.4.20 — Seed recetario tanda 10 española. admin SDK, SOLO-ALTA, idempotente.
 * Sin ingredientes nuevos: todas las refs usan el catálogo vivo.
 */
...
const NUEVOS_ING: any[] = []; // Sin ingredientes nuevos en tanda 10
const NUEVAS_REC = JSON.parse(readFileSync(resolve(DIR, "recetas_tanda10.json"), "utf-8")) as any[];
...
async function main() {
  ...
  writeFileSync(resolve(backupDir, `E3.4.20_ingredientes_${ts}.json`), JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2));
  writeFileSync(resolve(backupDir, `E3.4.20_recetas_${ts}.json`), JSON.stringify(recSnap.docs.map(d => d.data()), null, 2));
  ...
  // ── F2 ──────────────────────────────────────────────────────────────────
  console.log(`F2: 0 creados (sin ingredientes nuevos en tanda 10).`);
...
main().then(() => { console.log("Seed tanda 10 OK"); process.exit(0); }).catch((e) => { console.error(e); process.exit(1); });
