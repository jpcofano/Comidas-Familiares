...
// JSON files live directly in scripts/ (same convention as reseed-ingredientes.ts)
const NUEVOS_ING = JSON.parse(readFileSync(resolve("scripts/ingredientes_nuevos.json"), "utf-8")) as any[];
const NUEVAS_REC = JSON.parse(readFileSync(resolve("scripts/recetas_nuevas.json"), "utf-8")) as any[];
...
async function main() {
  ...
  writeFileSync(
    resolve(backupDir, `E3.4.9_ingredientes_${ts}.json`),
    JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2),
  );
  writeFileSync(
    resolve(backupDir, `E3.4.9_recetas_${ts}.json`),
...
main()
  .then(() => { console.log("Seed tanda 1 OK"); process.exit(0); })
