/**
 * Etapa E3.4.27 — Seed recetario tanda 17 peruana. admin SDK, SOLO-ALTA, idempotente.
 */
...
const NUEVOS_ING = JSON.parse(readFileSync(resolve(DIR, "ingredientes_nuevos_tanda17.json"), "utf-8")) as any[];
const NUEVAS_REC = JSON.parse(readFileSync(resolve(DIR, "recetas_tanda17.json"), "utf-8")) as any[];
...
async function main() {
  ...
  writeFileSync(resolve(backupDir, `E3.4.27_ingredientes_${ts}.json`), JSON.stringify(ingSnap.docs.map(d => d.data()), null, 2));
  writeFileSync(resolve(backupDir, `E3.4.27_recetas_${ts}.json`), JSON.stringify(recSnap.docs.map(d => d.data()), null, 2));
...
main().then(() => { console.log("Seed tanda 17 OK"); process.exit(0); }).catch((e) => { console.error(e); process.exit(1); });
