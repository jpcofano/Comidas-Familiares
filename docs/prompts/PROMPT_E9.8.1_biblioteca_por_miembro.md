...
### 0. Apertura del guard (alcance PRECISO — no abrir de más)

Hoy `BibliotecaRoute` (línea ~346) hace `if (!isJP) return <Navigate to="/" />`, bloqueando
**toda** la ruta a no-owner. **NO abrir la ruta entera.** Abrir solo:
- **`TabRecetas`** (lista de recetas, curada por miembro).
- **`DetalleReceta`** (para abrir una receta de su biblioteca).

**Sigue bloqueado a owner (isJP):** las demás tabs/secciones de Biblioteca (catálogo de
ingredientes, curación de visibilidad, import, edición). Mover el guard de nivel-ruta a
nivel-tab: un no-owner que entra a `/biblioteca` ve únicamente la tab Recetas; las tabs de
administración no se le renderizan. Acceso directo por URL a una sección de owner → redirect.

> Nota: se mantiene el patrón existente `isJP = memberId === "juanpablo"` (hardcodeado). Multi-familia
> NO es objetivo (si surge otra familia, se levanta instancia nueva), así que esto no es deuda técnica.

### 1. `Biblioteca.tsx` — `TabRecetas` filtra por miembro

- Obtener `memberId` del AuthProvider; owner = `memberId === "juanpablo"`.
