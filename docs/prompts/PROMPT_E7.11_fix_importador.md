...
- **Paso 3 (guardar):** crear las N recetas. Anti-duplicado por `nombreCanonico` / `idReceta` **por receta** (una duplicada no bloquea las demás). Reusar `crearReceta` en loop o batch.
...
### Implementación del vínculo de alternativa
Acá depende de lo que reportes en el punto 2 del reporte previo:
- **Si `IngredienteEnReceta` YA tiene un campo de alternativa/grupo:** usalo. Asigná a las dos filas el mismo `grupoAlternativa` (o el mecanismo que exista) y marcá `opcional: true` en ambas, o el patrón que ya use el modelo.
- **Si NO existe el campo:** agregá `alternativaDe?: string` (referencia al `ingredienteCanonico` o índice de la otra alternativa) a `IngredienteEnReceta` en `models.ts`, más lo necesario para que el detalle de receta y la lista de compras lo muestren como "Calvados *(o Sidra seca)*". Bumpea el shape en el MAPEO si tocás el modelo.

**Importante sobre la lista de compras:** dos alternativas NO deben sumar ambas a la lista de compras como ítems independientes a comprar. Definí el comportamiento mínimo: incluir solo la primera alternativa en compras, o agruparlas visualmente como "X o Y". Documentá la decisión que tomes. Si esto requiere tocar `src/data/compras.ts` y se complica, marcalo como sub-tarea y avisá antes — puede salir en commit aparte.
...
4. Ingrediente `Calvados o sidra seca` → dos `IngredienteEnReceta` vinculados como alternativa.
